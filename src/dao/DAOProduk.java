...
public class DAOProduk implements ServiceProduk {
    ...
    public void UpdateData(ModelProduk mopa) {
        String sql = "UPDATE menu_produk SET id_kategori=?, nama_menu=?, harga=?, stok=?, status=?  WHERE id='"+mopa.getId()+"'";
        try (PreparedStatement st = conn.prepareStatement(sql)) {
            st.setString(1, mopa.getIdKategori().getId());
            st.setString(2, mopa.getNamaMenu());
            st.setLong(3, mopa.getHarga());
...
//            System.out.println("ID UPDATE = " + mopa.getId());
    ...
    public List<ModelProduk> getData() {
        ...
        String sql = """
        SELECT mp.id, mp.id_kategori, mk.nama_kategori, mp.nama_menu, mp.harga, mp.stok, mp.status
        FROM menu_produk mp
